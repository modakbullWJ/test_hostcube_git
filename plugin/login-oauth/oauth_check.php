...
/*
 * 제대로 적용하려면 아래글 참고해서 DB의 필드수정도 필요합니다.
 * http://amina.co.kr/bbs/board.php?bo_table=tip&wr_id=2113
 * 위 내용적용시 아래 $mb_id = substr($mb_id,0,18); 코드는 주석처리해 주세요.
*/
...
if(popup.document.getElementsByName("slr_url").length) {
    url = popup.document.getElementsByName("slr_url")[0].value;
}

if(url) {
	popup.location.href = decodeURIComponent(url);
} else {
